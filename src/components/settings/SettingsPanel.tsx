...
export default function SettingsPanel({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60" data-context-scope="settings">
      <div className="bg-surface rounded-xl shadow-2xl w-full max-w-3xl h-[88vh] max-h-[88vh] overflow-hidden ring-1 ring-inset ring-primary-400/10 flex flex-col mx-4" data-context-scope="settings">
        ...
        <div className="flex border-b border-primary-400/10 shrink-0">
          {settingsTabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActiveTab(tab.id)}
              className={`flex-1 min-w-0 px-4 py-2.5 text-xs font-medium leading-tight transition-colors ${activeTab === tab.id
                  ? 'text-primary-300 border-b border-primary-400/20 bg-primary-500/6'
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overflow-x-hidden overscroll-x-none p-5 space-y-4">
          {activeTab === 'project' && (
            <SettingsProjectTab
