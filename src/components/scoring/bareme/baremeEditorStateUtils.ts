...
export function moveCategoryGroup(criteria: Criterion[], category: string, direction: 'up' | 'down') {
  ...
  const visibleOrder = Array.from(
    new Set(
      criteria.reduce<string[]>((acc, criterion) => {
        if (criterion.name.trim()) acc.push(getCriterionCategoryLabel(criterion))
        return acc
      }, []),
    ),
  )
...
export function validateCriteriaBeforeSave(criteria: Criterion[]) {
  const normalized = criteria.reduce<Criterion[]>((acc, criterion) => {
    const normalizedCriterion = normalizeCriterion(criterion)
    if (normalizedCriterion.name.trim()) acc.push(normalizedCriterion)
    return acc
  }, [])
