...
lazy_static! {
    ...
    static ref MEDIA_INFO_CACHE: Mutex<LruCache<super::mpv_wrapper::MediaInfo>> =
...
fn get_media_info_cached(path: &str) -> Option<super::mpv_wrapper::MediaInfo> {
    let key = media_info_cache_key(path);
    let mut cache = MEDIA_INFO_CACHE.lock().unwrap_or_else(|e| e.into_inner());
    cache.get(&key)
}

fn put_media_info_cache(path: &str, info: super::mpv_wrapper::MediaInfo) {
    let key = media_info_cache_key(path);
    let mut cache = MEDIA_INFO_CACHE.lock().unwrap_or_else(|e| e.into_inner());
    cache.put(key, info);
}

fn get_frame_preview_cached(path: &str, seconds: f64, width: u32) -> Option<String> {
    let key = frame_preview_cache_key(path, seconds, width);
    let mut cache = FRAME_PREVIEW_CACHE.lock().unwrap_or_else(|e| e.into_inner());
    cache.get(&key)
}

fn put_frame_preview_cache(path: &str, seconds: f64, width: u32, image: String) {
    let key = frame_preview_cache_key(path, seconds, width);
    let mut cache = FRAME_PREVIEW_CACHE.lock().unwrap_or_else(|e| e.into_inner());
    cache.put(key, image);
}
...
fn probe_media_info_via_mediainfo(path: &str) -> Result<super::mpv_wrapper::MediaInfo, String> {
    let mut command = Command::new(resolve_tool("mediainfo.exe"));
...
pub fn player_get_media_info(
    ...
    if let Some(target_path) = path {
        ...
        if !trimmed.is_empty() {
            ...
            if let Some(cached) = get_media_info_cached(&normalized_target) {
                return Ok(cached);
            }
            ...
            if let Ok(Ok(info)) = rx.recv_timeout(Duration::from_millis(2500)) {
                put_media_info_cache(&normalized_target, info.clone());
                return Ok(info);
            }
            ...
            let fallback = build_minimal_media_info(&trimmed);
            put_media_info_cache(&normalized_target, fallback.clone());
            return Ok(fallback);
        }
    }
    ...
    if let Ok(player) = state.player.try_lock() {
        if let Some(p) = &*player {
            ...
            let current_path = normalize_path(&p.get_current_path());
            if !current_path.is_empty() {
                put_media_info_cache(&current_path, info.clone());
            }
            return Ok(info);
        }
