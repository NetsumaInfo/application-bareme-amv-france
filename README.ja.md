...
- `useAppUpdateStore` — GitHub リリースによる更新チェック。署名付きの組み込み自動更新（tauri-plugin-updater）をアプリ内でダウンロード・インストール。
...
- 組み込み自動更新：署名付きの新バージョンがあるとヘッダーに青い「更新」ロゴが表示され、クリックするとプロジェクトを保存してからダウンロード・インストール・再起動を行う。
